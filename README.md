...
Development priorities and cross-computer handoff instructions are maintained
...
1. `Generate UIView Script` writes a deterministic `// <auto-generated />`
   view source to the configured Assets directory.
2. After Unity compiles, `Bind Generated UIView` adds the generated component
   and assigns every strongly typed reference.
3. `Preprocess Sorting Hierarchy` records all nested Canvas and Renderer nodes
   in their authored visual order and hierarchy depth.
